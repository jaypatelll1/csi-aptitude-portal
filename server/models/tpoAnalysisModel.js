  ...
  try {
    const { rows } = await dbWrite.raw(`
      SELECT 
        sa.department_name,
        ROUND(
          ((SUM(sa.total_score) * 100.0) / NULLIF(SUM(sa.max_score), 0))::NUMERIC,
        2) AS accuracy_rate
    ...
    const { rows } = await dbWrite.raw(`
      SELECT 
        ...
        ROUND(
          ...
          (SELECT COUNT(*) FROM users u WHERE u.department = sa.department_name),
...
const categoryWiseAccuracy = async () => {
  try {
    const { rows } = await dbWrite.raw(`
      ...
      FROM student_analysis sa,
      LATERAL jsonb_each(sa.category) AS cat
