...
const generateRank = async () => {
  try {
    ...
    results.map(async (ranking) => {
      let { student_id, department, rank, total_score } = ranking;

      queryText = `INSERT INTO student_rank (student_id, department_name, rank, total_score)
        VALUES ($1, $2, $3, $4);`;

      await query(queryText, [student_id, department, rank, total_score]);
    });
    return results;
  } catch (error) {
    console.log(error);
